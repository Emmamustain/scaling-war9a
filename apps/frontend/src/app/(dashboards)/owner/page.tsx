...
export default function OwnerIndexPage() {
  ...
  const { data: businesses, isLoading } = useQuery({
    ...
    queryFn: () => fetchApi<MyBusiness[]>("/businesses/mine"),
  });

  // Auto-redirect when there is exactly one business
  useEffect(() => {
    if (businesses?.length === 1) {
      router.replace(`/owner/${businesses[0].slug}`);
    }
  }, [businesses, router]);
  ...
  if (!businesses || businesses.length === 0) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <div>
          <h1 className="text-xl font-bold">No businesses yet</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            You don&apos;t own any registered businesses.
          </p>
        </div>
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-12">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Select a business to open its dashboard.
        </p>
      </div>

      <div className="space-y-3">
        {businesses.map((biz: MyBusiness) => (
          <button
            key={biz.id}
            onClick={() => router.push(`/owner/${biz.slug}`)}
