...
import AssignWorker from "../Molecules/AssignWorker";
...
interface HandleAssignWorkerProps {
  businessSlug: string;
  serviceId: string;
}

export default function HandleAssignWorker({
  businessSlug,
  serviceId,
}: HandleAssignWorkerProps) {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <Drawer open={isOpen}>
      <DrawerTrigger asChild onClick={() => setIsOpen((curr) => !curr)}>
        <Button className=" mt-8 w-72 bg-emerald-500 ">
          <PlusCircle size={20} className="m-2" /> Assign worker
        </Button>
      </DrawerTrigger>
      <DrawerContent>
